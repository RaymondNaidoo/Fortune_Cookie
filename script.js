...
// Ensure the paper width tracks text during typing
function syncPaperWidth() {
  // padding-left + padding-right = 32 (from CSS)
  const targetWidth = textEl.scrollWidth + 32;
  paper.style.width = targetWidth + "px";
}

// Typewriter
async function typeText(text) {
  textEl.textContent = "";
  paper.style.width = "32px";      // start minimal with padding only
  document.body.classList.add("typing");
  for (let i = 0; i <= text.length; i++) {
    textEl.textContent = text.slice(0, i);
    syncPaperWidth();              // adjust banner to current text
...
function resetCookie() {
  ...
  paper.style.width = "32px";      // reset width to padding baseline
